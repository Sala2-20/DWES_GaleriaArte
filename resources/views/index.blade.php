...
<script>
    ...
    //errores al crear usuarios
        ...
        function iniciar() {
            alert("Error al crear la obra" + '{{ $errors->first('error') }}');
        }
    @endif
</script>
@endsection
...
<div id="inicioSesion" class="contenedor" onclick="mostrarInicio()">
    ...
    <form action="{{route('log.store')}}" method="POST" id="inicio" style="display: none;">
        @csrf
        Nombre:
        <br>
...
<div id="registrarse" class="contenedor" onclick="mostrarRegis()">
    ...
    <form action="{{route('log.create')}}" method="POST" id="registro" style="display: none;">
        @csrf
        Nombre:
        <br>
